...
=begin
goal: find the lonest sentence in the text, based on number of words. Everything not a space, or end of a sentence is a word. Print the number of words in the longest sentence
...
def text(strs)
 ...
 p length_array = sentences.map { |str| str.split(" ").count}
  # length_array.index(length_array.max)
  length_array.max
  p sentence_return = sentences[length_array.max]

  puts "#{sentence_return}"
  puts "is #{length_array.max} words"
end

# p text("Four score and seven years ago our fathers brought forth
...
p text("You will rejoice to hear that no disaster has accompanied the
